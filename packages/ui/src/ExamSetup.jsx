...
import { RefreshCcw, User, Settings2, Clock, CheckSquare, Activity, ShieldCheck } from 'lucide-react';
...
const ExamSetup = ({
  ...
  const panelStyle = {
    ...
    borderRadius: '12px', padding: '20px', boxShadow: '0 4px 30px rgba(0,0,0,0.3)',
    backdropFilter: 'blur(8px)', height: '520px', display: 'flex', flexDirection: 'column'
  };
  ...
  const subjectsAvailable = Object.keys(subjectHierarchy).length > 0;
  const timePerQuestion = questionCount === 'all' ? "Variable" : Math.round((examDuration / questionCount) / 60 * 10) / 10 + " min";
    ...
    <div style={{ maxWidth: '1100px', margin: '20px auto 80px auto', width: '100%', padding: '0 20px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', gap: '15px' }}>
        <h2 style={{ color: '#fff', margin: 0, fontSize: '1.8rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Settings2 color="#38bdf8" /> Configure CBT
        </h2>
      ...
      <div style={{ ...panelStyle, height: 'auto', marginBottom: '20px', padding: '15px 20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px' }}>
          {Object.keys(subjectHierarchy).map(mainSubject => {
            ...
            return (
              <div key={mainSubject} onClick={() => setExpandedFolder(mainSubject)}
                   style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', padding: '10px', borderRadius: '8px', background: isExpanded ? 'rgba(56, 189, 248, 0.1)' : '#0f172a', border: `1px solid ${isExpanded ? '#38bdf8' : '#1e293b'}` }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
        ...
        <div style={panelStyle}>
           ...
           <div style={{ flex: 1, overflowY: 'auto', background: '#020617', borderRadius: '8px', padding: '10px', border: '1px solid #1e293b', marginBottom: '15px' }}>
              {expandedFolder ? subjectHierarchy[expandedFolder].map(sub => (
                <label key={sub} style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.9rem', color: '#cbd5e1', padding: '8px 5px', borderBottom: '1px solid rgba(255,255,255,0.03)', cursor: 'pointer' }}>
                  <input type="checkbox" checked={selectedSubTopics.includes(sub)} onChange={() => toggleSubTopic(sub)} style={{ accentColor: '#38bdf8', width: '16px', height: '16px' }} />
                  {sub}
                </label>
        ...
        {/* Center: Profile & Session Analytics */}
        <div style={{ ...panelStyle, justifyContent: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
            <div style={{ width: '90px', height: '90px', borderRadius: '50%', backgroundColor: '#1e293b', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #38bdf8', overflow: 'hidden' }}>
              {userData?.photoURL ? <img src={userData.photoURL} style={{width:'100%', height:'100%', objectFit:'cover'}} /> : <User size={40} color="#64748b" />}
            </div>
            
            <div style={{ width: '100%', textAlign: 'center' }}>
              <div style={{ padding: '10px', borderRadius: '8px', border: '1px solid #334155', backgroundColor: '#0f172a', color: '#38bdf8', fontWeight: 'bold', textTransform: 'uppercase' }}>
                {userData?.username || userData?.displayName || "CANDIDATE"}
              </div>
              <p style={{ color: '#64748b', fontSize: '11px', marginTop: '5px', textTransform: 'lowercase' }}>{userData?.email}</p>
            </div>

            <div style={{ width: '100%', background: 'rgba(16, 185, 129, 0.05)', border: '1px dashed rgba(16, 185, 129, 0.3)', borderRadius: '12px', padding: '15px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#10b981', fontSize: '0.75rem', fontWeight: 'bold', marginBottom: '8px' }}>
                <Activity size={16} /> SESSION ANALYTICS
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: '#cbd5e1', marginBottom: '4px' }}>
                <span>Est. Pacing:</span> <strong>{timePerQuestion} / Q</strong>
              </div>
            ...
            <button onClick={handleStartOfficialExam} disabled={isFetchingExam || selectedSubTopics.length === 0} 
              style={{ width: '100%', padding: '16px', borderRadius: '10px', border: 'none', background: selectedSubTopics.length === 0 ? '#334155' : 'linear-gradient(135deg, #0284c7, #0ea5e9)', color: 'white', fontSize: '1.1rem', fontWeight: 'bold', cursor: 'pointer' }}>
        ...
        <div style={panelStyle}>
          ...
          <div style={{ marginTop: 'auto' }}>
            <label style={labelStyle}>Randomization Engine</label>
            <div style={{ background: '#0f172a', padding: '15px', borderRadius: '10px', border: '1px solid #1e293b', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <label style={{ fontSize: '0.9rem', color: '#cbd5e1', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                <input type="checkbox" checked={shuffleQuestions} onChange={(e) => setShuffleQuestions(e.target.checked)} style={{ accentColor: '#38bdf8' }} /> Shuffle Q's
              </label>
              <label style={{ fontSize: '0.9rem', color: '#cbd5e1', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                <input type="checkbox" checked={shuffleOptions} onChange={(e) => setShuffleOptions(e.target.checked)} style={{ accentColor: '#38bdf8' }} /> Shuffle Opts
              </label>
            </div>
          </div>
        </div>
