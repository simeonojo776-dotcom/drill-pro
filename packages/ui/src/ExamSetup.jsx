...
import { RefreshCcw, User, Settings2, Clock, Activity, ShieldCheck, Camera, Zap, CheckCircle2, Circle } from 'lucide-react';
...
const ExamSetup = ({
  ...
  const panelStyle = { background: 'rgba(15, 23, 42, 0.6)', border: '1px solid rgba(56, 189, 248, 0.2)', borderRadius: '12px', padding: '20px', boxShadow: '0 4px 30px rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', height: '540px', display: 'flex', flexDirection: 'column' };
    ...
    <div style={{ maxWidth: '1100px', margin: '20px auto 80px auto', width: '100%', padding: '0 20px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
        ...
        <div style={panelStyle}>
           ...
           <div style={{ flex: 1, overflowY: 'auto', background: '#020617', borderRadius: '8px', padding: '12px', border: '1px solid #1e293b', marginBottom: '15px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {expandedFolder ? (
                <>
                  <div 
                    onClick={() => toggleEntireFolder(expandedFolder)}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 15px', borderRadius: '8px', cursor: 'pointer', background: isAllSelected ? 'rgba(56, 189, 248, 0.15)' : 'rgba(255,255,255,0.02)', border: `1px dashed ${isAllSelected ? '#38bdf8' : '#334155'}`, transition: 'all 0.2s', marginBottom: '5px' }}
                  >
                    <span style={{ color: isAllSelected ? '#38bdf8' : '#cbd5e1', fontSize: '0.9rem', fontWeight: 'bold' }}>Select All {expandedFolder}</span>
                    {isAllSelected ? <CheckCircle2 size={18} color="#38bdf8" /> : <Circle size={18} color="#475569" />}
                  </div>
                  ...
                  {subjectHierarchy[expandedFolder].map(sub => {
                    ...
                    return (
                      <div 
                        ...
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 15px', borderRadius: '8px', cursor: 'pointer', background: isSelected ? 'rgba(56, 189, 248, 0.1)' : '#0f172a', border: `1px solid ${isSelected ? '#38bdf8' : '#1e293b'}`, transition: 'all 0.2s' }}
                      >
                        <span style={{ color: isSelected ? '#fff' : '#94a3b8', fontSize: '0.85rem', paddingRight: '10px', lineHeight: '1.4' }}>{sub}</span>
                        <div style={{ flexShrink: 0 }}>
                          {isSelected ? <CheckCircle2 size={18} color="#38bdf8" /> : <Circle size={18} color="#334155" />}
                        </div>
                      </div>
                    );
                  })}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#475569', gap: '10px' }}>
                  <Settings2 size={32} color="#1e293b" />
                  <p style={{ margin: 0 }}>Select a subject folder above</p>
                </div>
              )}
           </div>
            ...
            <div style={{ width: '100%', background: 'rgba(56, 189, 248, 0.05)', border: '1px solid rgba(56, 189, 248, 0.2)', borderRadius: '12px', padding: '15px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div style={{ background: '#0f172a', padding: '10px', borderRadius: '8px', border: '1px solid #1e293b' }}>
                  <p style={{ margin: 0, fontSize: '0.7rem', color: '#94a3b8', textTransform: 'uppercase' }}>Questions</p>
                  <p style={{ margin: '2px 0 0 0', fontSize: '1rem', color: '#f8fafc', fontWeight: 'bold' }}>
                    {questionCount === 'all' ? 'Maximum' : questionCount}
                  </p>
                </div>
                
                <div style={{ background: '#0f172a', padding: '10px', borderRadius: '8px', border: '1px solid #1e293b' }}>
                  <p style={{ margin: 0, fontSize: '0.7rem', color: '#94a3b8', textTransform: 'uppercase' }}>Time Limit</p>
                  <p style={{ margin: '2px 0 0 0', fontSize: '1rem', color: '#f8fafc', fontWeight: 'bold' }}>
                    {examDuration === 0 ? "Untimed" : `${examDuration / 60} Mins`}
                  </p>
                </div>

                <div style={{ background: '#0f172a', padding: '10px', borderRadius: '8px', border: '1px solid #1e293b' }}>
                  <p style={{ margin: 0, fontSize: '0.7rem', color: '#94a3b8', textTransform: 'uppercase' }}>Est. Pacing</p>
                  <p style={{ margin: '2px 0 0 0', fontSize: '1rem', color: '#f8fafc', fontWeight: 'bold' }}>
                    {examDuration === 0 || questionCount === 'all' ? "N/A" : `${Math.round((examDuration / questionCount) / 60 * 10) / 10} min/Q`}
                  </p>
                </div>

                <div style={{ background: '#0f172a', padding: '10px', borderRadius: '8px', border: '1px solid #1e293b' }}>
                  <p style={{ margin: 0, fontSize: '0.7rem', color: '#94a3b8', textTransform: 'uppercase' }}>Conditions</p>
                  <p style={{ margin: '2px 0 0 0', fontSize: '0.9rem', color: strictMode && examDuration > 0 ? '#ef4444' : '#10b981', fontWeight: 'bold' }}>
                    {strictMode && examDuration > 0 ? "Strict Mode" : "Practice"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div style={{ marginTop: '0' }}>
            ...
            <div style={{ background: '#0f172a', padding: '15px', borderRadius: '10px', border: '1px solid #1e293b', display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <label style={{ fontSize: '0.85rem', color: '#cbd5e1', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><input type="checkbox" checked={shuffleQuestions} onChange={(e) => setShuffleQuestions(e.target.checked)} style={{ accentColor: '#38bdf8', width: '16px', height: '16px' }} /> Shuffle Q's</label>
                <label style={{ fontSize: '0.85rem', color: '#cbd5e1', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><input type="checkbox" checked={shuffleOptions} onChange={(e) => setShuffleOptions(e.target.checked)} style={{ accentColor: '#38bdf8', width: '16px', height: '16px' }} /> Shuffle Opts</label>
              </div>
              <hr style={{ borderTop: '1px solid #1e293b', borderBottom: 'none', margin: '0' }} />
              <label style={{ fontSize: '0.85rem', color: '#cbd5e1', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><input type="checkbox" checked={strictMode} onChange={(e) => setStrictMode && setStrictMode(e.target.checked)} disabled={examDuration === 0} style={{ accentColor: '#ef4444', width: '16px', height: '16px' }} /> Strict Mode {examDuration === 0 && "(Disabled)"}</label>
              <label style={{ fontSize: '0.85rem', color: '#cbd5e1', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><input type="checkbox" checked={instantFeedback} onChange={(e) => setInstantFeedback && setInstantFeedback(e.target.checked)} style={{ accentColor: '#10b981', width: '16px', height: '16px' }} /> <Zap size={14} color="#10b981" /> Show Instant Feedback</label>
            </div>
          </div>
        </div>
